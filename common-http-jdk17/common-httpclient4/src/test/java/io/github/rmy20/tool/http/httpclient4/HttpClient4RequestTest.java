package io.github.rmy20.tool.http.httpclient4;
 ...
 * @author sheng
 */
class HttpClient4RequestTest {
    private static final boolean skip = true;
    private static final String uri = "http://127.0.0.1:6021/";
    private static final Logger log = LoggerFactory.getLogger(HttpClient4RequestTest.class);
    private static final BaseRequestTool tool = HttpClient4RequestTool.create()
            .requestConfig(RequestConfig.custom().setSocketTimeout(30000).build());
    ...
    void download() {
        ...
        BaseExecutor<Long> executor = tool.get(uri).paths("download")
                ...
                .downloadExecutor(new File("/opt/httpclient4/" + "SM2公私钥对.txt")).execute();
    ...
    void downloadAsync() throws Exception {
        ...
        CompletableFuture<? extends BaseExecutor<Long>> future = tool.get(uri).paths("download")
                .setContentType(MediaType.APPLICATION_OCTET_STREAM)
                .downloadExecutor(new File("/opt/httpclient4/async/" + "SM2公私钥对.txt")).executeAsync();
        for (int i = 0; i < 20; i++) {
            log.info("{}", i);
            TimeUnit.MILLISECONDS.sleep(200L);
        }
