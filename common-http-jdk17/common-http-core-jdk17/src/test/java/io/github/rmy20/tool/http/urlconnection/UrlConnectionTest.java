package io.github.rmy20.tool.http.urlconnection;
...
import io.github.rmy20.tool.http.urlconnection.request.UrlConnectionMultipartRequest;
...
public class UrlConnectionTest {
    ...
    void get() throws Exception {
        ...
        Assert.isTrue(executor.isOk(), "UrlConnection get请求失败");
        System.out.println("UrlConnection get结果：" + executor.get());
    ...
    void getAsync() throws Exception {
        ...
        UrlConnectionExecutor<String> executor = future.join();
        Assert.isTrue(executor.isOk(), "UrlConnection get 异步请求失败");
        System.out.println("UrlConnection get 异步结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }
    ...
    void postAsync() throws Exception {
        ...
        UrlConnectionExecutor<Map<String, Object>> executor = future.join();
        Assert.isTrue(executor.isOk(), "UrlConnection post 异步请求失败");
        System.out.println("UrlConnection post 异步结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }
    ...
    void form() {
        ...
        UrlConnectionExecutor<String> executor = UrlConnectionTool.form(uri).paths("/test/form")
                ...
                .stringExecutor().execute();
        Assert.isTrue(executor.isOk(), "UrlConnection form 请求失败");
        System.out.println("UrlConnection form结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }
    ...
    void formAsync() throws Exception {
        ...
        CompletableFuture<UrlConnectionExecutor<String>> future = UrlConnectionTool.form(uri).paths("/test/form")
                .addTextEncoded("access_key", RandomUtil.generateUuidSimple())
                .addTextEncoded("biz_content", RandomUtil.generateUuidSimple())
                .addTextEncoded("format", "json")
                .addTextEncoded("request_id", RandomUtil.generateByTimeAndLetter(16, DateTool.yyyyMMddHHmmss))
        ...
        UrlConnectionExecutor<String> executor = future.join();
        Assert.isTrue(executor.isOk(), "UrlConnection form 异步请求失败");
        System.out.println("UrlConnection form 异步结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }
    ...
    void upload() throws Exception {
        ...
        UrlConnectionMultipartRequest multipartRequest = UrlConnectionTool.multipart(uri).path("upload").path("multi")
        ...
        UrlConnectionExecutor<String> executor = multipartRequest.stringExecutor().execute();
    ...
    void uploadAsync() throws Exception {
        ...
        UrlConnectionMultipartRequest multipartRequest = UrlConnectionTool.multipart(uri).path("upload").path("multi")
                ...
                .addText("format", "哈哈faga---===");
        base64FileMap.forEach((k, v) -> multipartRequest.addBinary("file", k, Base64.getDecoder().decode(v.getBytes(StandardCharsets.UTF_8))));
        CompletableFuture<UrlConnectionExecutor<String>> future = multipartRequest.stringExecutor().executeAsync();
        for (int i = 0; i < 20; i++) {
            System.out.println(i);
            TimeUnit.MILLISECONDS.sleep(200L);
        }
