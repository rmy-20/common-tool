package io.github.rmy20.tool.http.jdkclient;

import com.fasterxml.jackson.core.type.TypeReference;
import io.github.rmy20.tool.core.date.DateTool;
import io.github.rmy20.tool.core.lang.Assert;
import io.github.rmy20.tool.core.util.RandomUtil;
import io.github.rmy20.tool.http.core.MediaType;
import io.github.rmy20.tool.http.core.execute.BaseExecutor;
import io.github.rmy20.tool.http.core.result.HttpJsonResultHandle;
import io.github.rmy20.tool.http.jdkclient.executor.JdkClientAsyncExecutor;
import io.github.rmy20.tool.http.jdkclient.executor.JdkClientExecutor;
import io.github.rmy20.tool.http.jdkclient.request.JdkClientMultipartRequest;
import io.github.rmy20.tool.jackson.v2.JsonUtil;
import org.junit.jupiter.api.Test;

import java.io.File;
import java.nio.charset.StandardCharsets;
import java.time.Instant;
import java.util.Base64;
import java.util.HashMap;
import java.util.Map;
import java.util.concurrent.CompletableFuture;
import java.util.concurrent.TimeUnit;

/**
 * jdk httpclient 测试类
 *
 * @author sheng
 */
public class JdkClientTest {
    static final boolean skip = true;
    static final String uri = "http://127.0.0.1:6021";

    @Test
    void get() throws Exception {
        if (skip) {
            return;
        }
        JdkClientExecutor<String> executor = JdkClientTool.get(uri).paths("/test/get")
                .setContentType(MediaType.APPLICATION_JSON)
                .queryEncoded("name", "小明")
                .queryEncoded("age", 18)
                .queryEncoded("sex", "男")
                .queryEncoded("address", "中国")
                .stringExecutor().execute();
        Assert.isTrue(executor.isOk(), "JdkClient get请求失败");
        System.out.println("JdkClient get结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }

    @Test
    void getAsync() throws Exception {
        if (skip) {
            return;
        }
        CompletableFuture<JdkClientAsyncExecutor<String>> future = JdkClientTool.get(uri).paths("/test/get")
                .setContentType(MediaType.APPLICATION_JSON)
                .queryEncoded("name", "小明")
                .queryEncoded("age", 18)
                .queryEncoded("sex", "男")
                .queryEncoded("address", "中国")
                .stringExecutor().executeAsync();

        for (int i = 0; i < 20; i++) {
            System.out.println(i);
            TimeUnit.MILLISECONDS.sleep(200L);
        }

        JdkClientAsyncExecutor<String> executor = future.join();
        Assert.isTrue(executor.isOk(), "JdkClient get 异步请求失败");
        System.out.println("JdkClient get 异步结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }

    @Test
    void post() {
        if (skip) {
            return;
        }
        Map<String, Object> map = new HashMap<>(16);
        map.put("appId", "appId.h5");
        map.put("method", "apps.digit.line");
        map.put("signType", "NA");
        map.put("timestamp", Instant.now().toEpochMilli());
        map.put("sessionId", "中 文 哈哈~！@#￥%……&*（）——+{}《》？~!@#$%^&*()_+");
        map.put("version", "1.0");
        JdkClientExecutor<Map<String, Object>> executor = JdkClientTool.post(uri).pathsEncoded("/test/post")
                .setContentType(MediaType.APPLICATION_JSON_UTF8)
                .body(JsonUtil.toJson(map))
                .executor(HttpJsonResultHandle.create(JsonUtil.JSON_TOOL, new TypeReference<Map<String, Object>>() {
                })).mustHandleResult().execute();
        Assert.isTrue(executor.isOk(), "JdkClient post请求失败");
        System.out.println("JdkClient post结果：" + JsonUtil.toJson(executor.get()));
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }

    @Test
    void postAsync() throws Exception {
        if (skip) {
            return;
        }
        Map<String, Object> map = new HashMap<>(16);
        map.put("appId", "appId.h5");
        map.put("method", "apps.digit.line");
        map.put("signType", "NA");
        map.put("timestamp", Instant.now().toEpochMilli());
        map.put("sessionId", "");
        map.put("version", "1.0");
        CompletableFuture<JdkClientAsyncExecutor<Map<String, Object>>> future = JdkClientTool.post(uri).pathsEncoded("/test/post")
                .setContentType(MediaType.APPLICATION_JSON_UTF8)
                .body(JsonUtil.toJson(map))
                .jsonExecutor(HttpJsonResultHandle.create(JsonUtil.JSON_TOOL, new TypeReference<Map<String, Object>>() {
                })).executeAsync();
        for (int i = 0; i < 20; i++) {
            System.out.println(i);
            TimeUnit.MILLISECONDS.sleep(200L);
        }
        JdkClientAsyncExecutor<Map<String, Object>> executor = future.join();
        Assert.isTrue(executor.isOk(), "JdkClient post 异步请求失败");
        System.out.println("JdkClient post 异步结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }

    @Test
    void download() {
        if (skip) {
            return;
        }
        JdkClientExecutor<Long> executor = JdkClientTool.get(uri).paths("download")
                .setContentType(MediaType.APPLICATION_OCTET_STREAM)
                .downloadExecutor(new File("/opt/JdkClient/" + "SM2公私钥对.txt")).execute();
        Assert.isTrue(executor.isOk() && executor.get() >= 0, "JdkClient 下载文件失败");
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }

    @Test
    void downloadAsync() throws Exception {
        if (skip) {
            return;
        }
        CompletableFuture<JdkClientAsyncExecutor<Long>> future = JdkClientTool.get(uri).paths("download")
                .setContentType(MediaType.APPLICATION_OCTET_STREAM)
                .downloadExecutor(new File("/opt/JdkClient/async/" + "SM2公私钥对.txt")).executeAsync();
        for (int i = 0; i < 20; i++) {
            System.out.println(i);
            TimeUnit.MILLISECONDS.sleep(200L);
        }
        JdkClientAsyncExecutor<Long> executor = future.join();
        Assert.isTrue(executor.isOk() && executor.get() >= 0, "JdkClient 异步下载文件失败");
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }

    @Test
    void form() {
        if (skip) {
            return;
        }
        JdkClientExecutor<String> executor = JdkClientTool.form(uri).paths("/test/form")
                .addTextEncoded("access_key", RandomUtil.generateUuidSimple())
                .addTextEncoded("biz_content", RandomUtil.generateUuidSimple())
                .addTextEncoded("format", "json")
                .addTextEncoded("request_id", RandomUtil.generateByTimeAndLetter(16, DateTool.yyyyMMddHHmmss))
                .addTextEncoded("timestamp", Instant.now().toEpochMilli())
                .addTextEncoded("version", "版 本 号~!@#$%^&*()_+{}:;',./")
                .addTextEncoded("sign", "MEUCIQC5er362TvTWrTpoZzvYeHldXTJtEIZpZJOea6nDseHngIgV61eTm/R7XLOd4/9lWV9lbRQJFEhTxASWfWkqE65F5c=")
                .stringExecutor().execute();
        Assert.isTrue(executor.isOk(), "JdkClient form 请求失败");
        System.out.println("JdkClient form结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }

    @Test
    void formAsync() throws Exception {
        if (skip) {
            return;
        }
        CompletableFuture<JdkClientAsyncExecutor<String>> future = JdkClientTool.form(uri).paths("/test/form")
                .addTextEncoded("access_key", RandomUtil.generateUuidSimple())
                .addTextEncoded("biz_content", RandomUtil.generateUuidSimple())
                .addTextEncoded("format", "json")
                .addTextEncoded("request_id", RandomUtil.generateByTimeAndLetter(16, DateTool.yyyyMMddHHmmss))
                .addTextEncoded("timestamp", Instant.now().toEpochMilli())
                .addTextEncoded("version", "版 本 号~!@#$%^&*()_+{}:;',./")
                .addTextEncoded("sign", "MEUCIQC5er362TvTWrTpoZzvYeHldXTJtEIZpZJOea6nDseHngIgV61eTm/R7XLOd4/9lWV9lbRQJFEhTxASWfWkqE65F5c=")
                .stringExecutor().executeAsync();
        for (int i = 0; i < 20; i++) {
            System.out.println(i);
            TimeUnit.MILLISECONDS.sleep(200L);
        }
        JdkClientAsyncExecutor<String> executor = future.join();
        Assert.isTrue(executor.isOk(), "JdkClient form 异步请求失败");
        System.out.println("JdkClient form 异步结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }

    @Test
    void upload() throws Exception {
        if (skip) {
            return;
        }
        Map<String, String> base64FileMap = new HashMap<>(8);
        base64FileMap.put("t2.txt", "5aSp");
        base64FileMap.put("SM2公私钥对.txt",
                "");
        base64FileMap.put("test.html", "PCFET0NUWVBFIGh0bWw+DQo8aHRtbCBsYW5nPSJlbiI+DQo8aGVhZD4NCiAgICA8bWV0YSBjaGFyc2V0PSJVVEYtOCI" +
                "+DQogICAgPHRpdGxlPlRpdGxlPC90aXRsZT4NCjwvaGVhZD4NCjxib2R5Pg0KICAxDQo8L2JvZHk+DQo8L2h0bWw+");
        base64FileMap.put("t.docx", "UEsDBAoAAAAAAIdO4kAAAAAAAAAAAAAAAAAJAAAAZG9jUHJvcHMvUEsDBBQAAAAIAIdO4kC1" +
                "/ZeoWwEAAHECAAAQAAAAZG9jUHJvcHMvYXBwLnhtbJ1Ry26DMBC8V+o/IO5gQ0JeMkQpaU5VGwnaHCPLbIJVsC3biZK/rwlVQq" +
                "+97czujmZ3yPLSNt4ZtOFSpH4UYt8DwWTFxTH1P8tNMPM9Y6moaCMFpP4VjL/Mnp" +
                "/IVksF2nIwnpMQJvVra9UCIcNqaKkJXVu4zkHqlloH9RHJw4EzWEt2akFYFGM8QXCxICqoAnUX9HvFxdn+V7SSrPNnvsqrcoYzUkKrGmohe" +
                "+/sNGElbUvQnSVbegSTRQT1BdlJXd1wX5C8ppoy6/7UDQ0QeePCbWKC+sIpaXrUVNU3coBIKS1tSt5CN/0ApGC0gdzZzQ60MUDQg+jUv82nKuW6M//b/0sOvO24rQtFWX" +
                "/Kw+WAJyulGs6odXlnu23hfdwy2Udx6MIPo/l0iveb6HUUT1/yIJ7M82A8SqpgFSVxgJM8GeMZxnG" +
                "");
        JdkClientMultipartRequest multipartRequest = JdkClientTool.multipart(uri).path("upload").path("multi")
                .publisherType(JdkClientMultipartRequest.PublisherEnum.BOUNDED_QUEUE)
                .addText("bucketName", "local")
                .addText("oosType", "qn")
                .addText("md5", RandomUtil.generateUuidSimple())
                .addText("format", "哈哈faga---===");
        base64FileMap.forEach((k, v) -> multipartRequest.addBinary("file", k, Base64.getDecoder().decode(v.getBytes(StandardCharsets.UTF_8))));
        JdkClientExecutor<String> executor = multipartRequest.stringExecutor().execute();
        Assert.isTrue(executor.isOk(), "UrlConnection上传失败");
        System.out.println("UrlConnection上传结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }

    @Test
    void uploadAsync() throws Exception {
        if (skip) {
            return;
        }
        Map<String, String> base64FileMap = new HashMap<>(8);
        base64FileMap.put("t2.txt", "5aSp");
        base64FileMap.put("SM2公私钥对.txt",
                "");
        base64FileMap.put("test.html", "PCFET0NUWVBFIGh0bWw+DQo8aHRtbCBsYW5nPSJlbiI+DQo8aGVhZD4NCiAgICA8bWV0YSBjaGFyc2V0PSJVVEYtOCI" +
                "+DQogICAgPHRpdGxlPlRpdGxlPC90aXRsZT4NCjwvaGVhZD4NCjxib2R5Pg0KICAxDQo8L2JvZHk+DQo8L2h0bWw+");
        base64FileMap.put("t.docx", "UEsDBAoAAAAAAIdO4kAAAAAAAAAAAAAAAAAJAAAAZG9jUHJvcHMvUEsDBBQAAAAIAIdO4kC1" +
                "/ZeoWwEAAHECAAAQAAAAZG9jUHJvcHMvYXBwLnhtbJ1Ry26DMBC8V+o/IO5gQ0JeMkQpaU5VGwnaHCPLbIJVsC3biZK/rwlVQq" +
                "+97czujmZ3yPLSNt4ZtOFSpH4UYt8DwWTFxTH1P8tNMPM9Y6moaCMFpP4VjL/Mnp" +
                "/IVksF2nIwnpMQJvVra9UCIcNqaKkJXVu4zkHqlloH9RHJw4EzWEt2akFYFGM8QXCxICqoAnUX9HvFxdn+V7SSrPNnvsqrcoYzUkKrGmohe" +
                "+/sNGElbUvQnSVbegSTRQT1BdlJXd1wX5C8ppoy6/7UDQ0QeePCbWKC+sIpaXrUVNU3coBIKS1tSt5CN/0ApGC0gdzZzQ60MUDQg+jUv82nKuW6M//b/0sOvO24rQtFWX" +
                "/Kw+WAJyulGs6odXlnu23hfdwy2Udx6MIPo/l0iveb6HUUT1/yIJ7M82A8SqpgFSVxgJM8GeMZxnG" +
                "");
        JdkClientMultipartRequest multipartRequest = JdkClientTool.multipart(uri).path("upload").path("multi")
                .addText("bucketName", "local")
                .addText("oosType", "qn")
                .addText("md5", RandomUtil.generateUuidSimple())
                .addText("format", "哈哈faga---===");
        base64FileMap.forEach((k, v) -> multipartRequest.addBinary("file", k, Base64.getDecoder().decode(v.getBytes(StandardCharsets.UTF_8))));
        CompletableFuture<JdkClientAsyncExecutor<String>> future = multipartRequest.stringExecutor().executeAsync();
        for (int i = 0; i < 20; i++) {
            System.out.println(i);
            TimeUnit.MILLISECONDS.sleep(200L);
        }
        BaseExecutor<String> executor = future.join();
        Assert.isTrue(executor.isOk(), "UrlConnection 异步上传失败");
        System.out.println("UrlConnection 异步上传结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }
}
