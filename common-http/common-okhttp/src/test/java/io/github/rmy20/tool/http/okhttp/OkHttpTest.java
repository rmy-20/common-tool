...
import io.github.rmy20.tool.http.okhttp.executor.OkHttpAsyncExecutor;
import io.github.rmy20.tool.http.okhttp.executor.OkHttpExecutor;
import io.github.rmy20.tool.http.okhttp.request.OkHttpMultipartRequest;
...
class OkHttpTest {
    ...
    void get() {
        ...
        OkHttpExecutor<String> executor = OkHttpTool.get(uri).paths("/test/get")
    ...
    void postAsync() throws Exception {
        ...
        OkHttpAsyncExecutor<Map<String, Object>> executor = future.join();
        Assert.isTrue(executor.isOk(), "okhttp post请求失败");
        System.out.println("okhttp post结果：" + executor.get());
        executor.getHeaders().forEach((name, value) -> System.out.println("header --> " + name + "：" + value));
    }
    ...
    void upload() throws Exception {
        ...
        OkHttpMultipartRequest multipartRequest = OkHttpTool.multipart(uri).path("upload").path("multi")
        ...
        OkHttpExecutor<String> executor = multipartRequest.stringExecutor().execute();
