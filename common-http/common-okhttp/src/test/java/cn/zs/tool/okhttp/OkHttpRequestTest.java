...
import cn.zs.tool.okhttp.request.OkHttpRequestTool;
...
class OkHttpRequestTest {
    ...
    void get() throws Exception {
        ...
        Assert.isTrue(executor.isOk(), "okhttp get请求失败");
        System.out.println("okhttp get结果：" + executor.get());
    ...
    void getAsync() throws Exception {
        ...
        Assert.isTrue(executor.isOk(), "okhttp get 异步请求失败");
        System.out.println("okhttp get 异步结果：" + executor.get());
        executor.getHeaders().forEach((key, value) -> log.info("header --> {}：{}", key, value));
    }
    ...
    void postAsync() throws Exception {
        ...
        Assert.isTrue(executor.isOk(), "okhttp post 异步请求失败");
        System.out.println("okhttp post 异步结果：" + executor.get());
        executor.getHeaders().forEach((key, value) -> log.info("header --> {}：{}", key, value));
    }
    ...
    void download() {
        ...
        BaseExecutor<Boolean> executor = tool.get(uri).paths("download")
                ...
                .downloadExecutor(new File("/opt/okhttp/" + "SM2公私钥对.txt")).execute();
    ...
    void downloadAsync() throws Exception {
        ...
        CompletableFuture<? extends BaseExecutor<Boolean>> future = tool.get(uri).paths("download")
                .setContentType(MediaTypeEnum.APPLICATION_OCTET_STREAM.getMediaType())
                .downloadExecutor(new File("/opt/okhttp/async/" + "SM2公私钥对.txt")).executeAsync();
        for (int i = 0; i < 20; i++) {
            log.info("{}", i);
            TimeUnit.MILLISECONDS.sleep(200L);
        }
        BaseExecutor<Boolean> executor = future.join();
        Assert.isTrue(executor.isOk() && executor.get(), "okhttp 异步下载文件失败");
        executor.getHeaders().forEach((key, value) -> log.info("header --> {}：{}", key, value));
    }
    ...
    void formAsync() throws Exception {
        ...
        BaseExecutor<String> executor = future.join();
        Assert.isTrue(executor.isOk(), "okhttp form 异步请求失败");
        System.out.println("okhttp form 异步结果：" + executor.get());
        executor.getHeaders().forEach((key, value) -> log.info("header --> {}：{}", key, value));
    }
    ...
    void upload() throws Exception {
        ...
        Assert.isTrue(executor.isOk(), "okhttp上传失败");
        System.out.println("okhttp上传结果：" + executor.get());
        executor.getHeaders().forEach((key, value) -> log.info("header --> {}：{}", key, value));
    }
    ...
    void uploadAsync() throws Exception {
        ...
        BaseExecutor<String> executor = future.join();
        Assert.isTrue(executor.isOk(), "okhttp 异步上传失败");
        System.out.println("okhttp 异步上传结果：" + executor.get());
        executor.getHeaders().forEach((key, value) -> log.info("header --> {}：{}", key, value));
    }
}
