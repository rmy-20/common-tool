...
class OkHttpTest {
    ...
    void postAsync() throws Exception {
        ...
        OkHttpAsyncExecutor<Map<String, Object>> exchange = future.join();
        Assert.isTrue(exchange.isOk(), "okhttp post请求失败");
    ...
    void form() {
        ...
        OkHttpExecutor<String> exchange = OkHttpTool.form(uri).paths("/test/form")
                .addEncoded("access_key", RandomUtil.generateUuidSimple())
                .addEncoded("biz_content", RandomUtil.generateUuidSimple())
                .addEncoded("format", "json")
                .addEncoded("request_id", RandomUtil.generateByTimeAndLetter(16, DateTool.yyyyMMddHHmmss))
                .addEncoded("timestamp", Instant.now().toEpochMilli())
                .addEncoded("version", "版本号")
                .addEncoded("sign", "MEUCIQC5er362TvTWrTpoZzvYeHldXTJtEIZpZJOea6nDseHngIgV61eTm/R7XLOd4/9lWV9lbRQJFEhTxASWfWkqE65F5c=")
                .executeForString();
    ...
    void upload() throws Exception {
        ...
        OkHttpMultipartRequest multipartRequest = OkHttpTool.multipart(uri).path("upload").path("multi")
                .add("bucketName", "local")
                .add("oosType", "qn")
                .add("md5", RandomUtil.generateUuidSimple())
                .add("format", "哈哈faga---===");
        base64FileMap.forEach((k, v) -> multipartRequest.add("file", k, Base64.getDecoder().decode(v.getBytes(StandardCharsets.UTF_8))));
