        ...
        Arrays.stream(SymmetricCryptoTool.values()).forEach(tool -> {
            ...
            Stream.of(ModeEnum.CBC, ModeEnum.CFB, ModeEnum.CTR, ModeEnum.CTS, ModeEnum.ECB, ModeEnum.OFB).forEach(mode -> {
                Stream.of(PaddingEnum.PKCS5Padding, PaddingEnum.PKCS7Padding).forEach(padding -> {
                    ...
                    Assert.isTrue(plainText.equals(tool.decryptHex(encrypt, secretKey, mode.isNeedIv() ? iv : null, mode, padding)),
                            algorithmName + " hex解密失败");
                    ...
                    Assert.isTrue(plainText.equals(tool.decryptBase64(encrypt, secretKey, mode.isNeedIv() ? iv : null, mode, padding)),
                            algorithmName + " base64解密失败");
                    System.out.println();
                });
            });
            ...
            if (SymmetricCryptoTool.SM4 == tool || SymmetricCryptoTool.AES == tool) {
                ...
                System.out.println(algorithmName + "    hex   " + encrypt);
                Assert.isTrue(plainText.equals(tool.decryptHex(encrypt, secretKey, iv, cipherAdditional, ModeEnum.GCM, PaddingEnum.NoPadding)),
                        algorithmName + " hex解密失败");

                encrypt = tool.encryptBase64(plainText, secretKey, iv, cipherAdditional, ModeEnum.GCM, PaddingEnum.NoPadding);
                System.out.println(algorithmName + "    base64   " + encrypt);
                Assert.isTrue(plainText.equals(tool.decryptBase64(encrypt, secretKey, iv, cipherAdditional, ModeEnum.GCM, PaddingEnum.NoPadding)),
                        algorithmName + " base64解密失败");
                System.out.println();
            }
        });
